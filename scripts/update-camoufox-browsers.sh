#!/usr/bin/env bash
# Usage: ./scripts/update-camoufox-browsers.sh [version]
#
# Add Camoufox <version> (default: latest daijro/camoufox GitHub release) to
# pins/camoufox-browsers/. Prefetches supported browser archive hashes, writes the pin
# file, updates pins/pin.json, builds the versioned attr, and commits.

set -euo pipefail

TOOL="camoufox-browsers"
...
(
  ...
  git commit -m "camoufox-browsers: add ${package_version}"
)
log "done. added camoufox-browsers-${package_version}"
